//Keerthana Pullela, Khalil Scott-Shepherd, Jamarri White
...
{ 
    ...
    //  Traverse until all processes are executed.
...
// sort the Process acc. to priority
...
void printMetrics(ProcessType plist[], int n)
{
    ...
    for (int  x=0; x<n; x++) 
    { 
        total_wt = total_wt + plist[x].wt; 
        total_tat = total_tat + plist[x].tat; 
        printf("\t%d\t\t%d\t\t%d\t\t%d\n", plist[x].pid, plist[x].bt, plist[x].wt, plist[x].tat); 
    } 
